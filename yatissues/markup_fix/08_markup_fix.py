"""
Markup fixer + audit for yat.txt (Wil-YAT).
Counterpart of PWG#175, PWK#113, and siblings.

Wil-YAT-specific notes: No whitespace hits. Essentially clean.

Inputs: ../../../csl-orig/v02/yat/yat.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "yat" / "yat.txt", HERE / "yat.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "yat_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(F|lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for yat.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"Wil-YAT markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against yat.txt.\n")
        f.write(f"Paired tags: <F>, <lang>\n")
        f.write(f"Notes: No whitespace hits. Essentially clean.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <F>, <lang>\n\n")
