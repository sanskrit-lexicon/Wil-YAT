...
def extract2_recs(filein):
 ...
 wilFlag = (filein == 'wilhw2.txt') 
 if wilFlag:
  print "extract2_recs found Wilson dictionary"
 recs=[]
 n = 0
 ndup = 0
 recprev = None
 for line in f:
  ...
  if n == 0:
   ...
   if wilFlag and rec.hw.endswith('a') and (rec.hw != 'a'):
    recprev = rec  # Not a dup (1st is verb, 2nd is noun)
    recs.append(rec)
   else: # a genuine dup
    recs.append(rec)
    ndup = ndup + 1
